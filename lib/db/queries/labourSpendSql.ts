...
// stored salary_amount → people_count × wage_per_head, plus the overtime lump
// sum (ot_total_amount) on top of whichever branch won.
//
...
export const labourSpendSumExpr = sql`sum(case
      ...
      end + coalesce(ot_total_amount,0))`;

// The overtime share of labourSpendSumExpr, for summaries that show OT as its
// own line. It is already included in labourSpendSumExpr — never add the two.
export const labourOtSumExpr = sql`sum(coalesce(ot_total_amount,0))`;
